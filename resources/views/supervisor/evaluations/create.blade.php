...
@section('title', isset($evaluation) ? 'Edit Performance Evaluation' : 'Create Performance Evaluation')
...
@section('breadcrumb')
    ...
    <span class="separator">/</span>
    <a href="{{ route('supervisor.trainees.show', $trainee->id) }}">{{ $trainee->fname }}</a>
    <span class="separator">/</span>
    <span class="current">{{ isset($evaluation) ? 'Edit' : 'New' }} Performance Evaluation</span>
@endsection

@section('content')
<div class="page-header">
    <h1 class="page-title">{{ isset($evaluation) ? 'Edit' : 'Create' }} Performance Evaluation</h1>
    <p class="page-subtitle">for {{ $trainee->full_name }}</p>
</div>
...
<form method="POST" action="{{ isset($evaluation) ? route('supervisor.evaluations.update', $evaluation->id) : route('supervisor.evaluations.store') }}">
    @csrf
        ...
        <div class="card-body" style="padding:24px;">
            <div style="margin-bottom:24px;">
                ...
                <input type="date" name="evaluation_date" 
                    value="{{ isset($evaluation) ? $evaluation->evaluation_date->format('Y-m-d') : request('date', today()->format('Y-m-d')) }}"
        ...
        <div class="card-body" style="padding:24px;">
            <div style="margin-bottom:24px;">
                ...
                <textarea name="strengths" rows="4" required minlength="10"
                    ...
                    style="width:100%; padding:12px 14px; border:1px solid var(--gray-200); border-radius:6px; font-size:0.9rem; font-family:inherit;">{{ isset($evaluation) ? $evaluation->strengths : '' }}</textarea>
                @error('strengths')
                    <span style="color:var(--danger); font-size:0.85rem; margin-top:4px; display:block;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="margin-bottom:24px;">
                ...
                <textarea name="areas_for_improvement" rows="4" required minlength="10"
                    ...
                    style="width:100%; padding:12px 14px; border:1px solid var(--gray-200); border-radius:6px; font-size:0.9rem; font-family:inherit;">{{ isset($evaluation) ? $evaluation->areas_for_improvement : '' }}</textarea>
            ...
            <div style="margin-bottom:24px;">
                ...
                <textarea name="skills_to_develop" rows="4" required minlength="10"
                    ...
                    style="width:100%; padding:12px 14px; border:1px solid var(--gray-200); border-radius:6px; font-size:0.9rem; font-family:inherit;">{{ isset($evaluation) ? $evaluation->skills_to_develop : '' }}</textarea>
            ...
            <div style="margin-bottom:24px;">
                ...
                <textarea name="overall_comments" rows="3" maxlength="1500"
                    ...
                    style="width:100%; padding:12px 14px; border:1px solid var(--gray-200); border-radius:6px; font-size:0.9rem; font-family:inherit;">{{ isset($evaluation) ? $evaluation->overall_comments : '' }}</textarea>
                    ...
                    <div style="display:flex; gap:10px; align-items:center;">
                        @for($i = 1; $i <= 5; $i++)
                            <input type="radio" id="technical_{{ $i }}" name="technical_skills_rating" value="{{ $i }}"
                                {{ isset($evaluation) && $evaluation->technical_skills_rating == $i ? 'checked' : '' }}
                    ...
                    <div style="display:flex; gap:10px; align-items:center;">
                        @for($i = 1; $i <= 5; $i++)
                            <input type="radio" id="communication_{{ $i }}" name="communication_rating" value="{{ $i }}"
                                {{ isset($evaluation) && $evaluation->communication_rating == $i ? 'checked' : '' }}
                                required style="cursor:pointer;">
                    ...
                    <div style="display:flex; gap:10px; align-items:center;">
                        @for($i = 1; $i <= 5; $i++)
                            <input type="radio" id="teamwork_{{ $i }}" name="teamwork_rating" value="{{ $i }}"
                                {{ isset($evaluation) && $evaluation->teamwork_rating == $i ? 'checked' : '' }}
                                required style="cursor:pointer;">
                    ...
                    <div style="display:flex; gap:10px; align-items:center;">
                        @for($i = 1; $i <= 5; $i++)
                            <input type="radio" id="professionalism_{{ $i }}" name="professionalism_rating" value="{{ $i }}"
                                {{ isset($evaluation) && $evaluation->professionalism_rating == $i ? 'checked' : '' }}
                                required style="cursor:pointer;">
                    ...
                    <div style="display:flex; gap:10px; align-items:center;">
                        @for($i = 1; $i <= 5; $i++)
                            <input type="radio" id="initiative_{{ $i }}" name="initiative_rating" value="{{ $i }}"
                                {{ isset($evaluation) && $evaluation->initiative_rating == $i ? 'checked' : '' }}
                                required style="cursor:pointer;">
    ...
    <div style="display:flex; gap:12px; justify-content:flex-end;">
        <a href="{{ route('supervisor.trainees.show', $trainee->id) }}" class="btn btn-ghost">Cancel</a>
        <button type="submit" class="btn btn-primary">
            {{ isset($evaluation) ? 'Update Evaluation' : 'Create Evaluation' }}
        </button>
    </div>
</form>
