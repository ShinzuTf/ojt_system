...
<div class="container-fluid">
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @if($issue->transfer_certificate_path)
                    <div class="mb-4">
                        <label class="form-label text-muted">Transfer Certificate</label>
                        <div class="p-3" style="background: #f8f9fa; border-radius: 0.375rem; display:flex; align-items:center; justify-content:space-between; gap:12px;">
                            <div>
                                <p class="mb-0"><strong>{{ $issue->transfer_certificate_name ?? basename($issue->transfer_certificate_path) }}</strong></p>
                                <small class="text-muted">Uploaded to document the company change and continue DTR tracking.</small>
                            </div>
                            <a href="{{ asset('storage/' . $issue->transfer_certificate_path) }}" class="btn btn-sm btn-outline-primary" download>
                                Download
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 pt-4 border-top">
                        ...
                        <form method="POST" action="{{ route('coordinator.issues.update', $issue->id) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label for="student_status" class="form-label">Student Status Update</label>
                                <select name="student_status" id="student_status" class="form-control">
                                    <option value="active" {{ $issue->student_status === 'active' ? 'selected' : '' }}>Active</option>
                                    <option value="dropped" {{ $issue->student_status === 'dropped' ? 'selected' : '' }}>Dropped</option>
                                    <option value="transferred" {{ $issue->student_status === 'transferred' ? 'selected' : '' }}>Transferred to another company</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label for="transfer_certificate" class="form-label">Transfer Certificate</label>
                                <input type="file" name="transfer_certificate" id="transfer_certificate" class="form-control">
                                <small class="text-muted">Required when the student is transferred to a new company so DTR can continue.</small>
                            </div>
        ...
        <div class="col-lg-4">
            ...
            <a href="{{ route('coordinator.issues.index') }}" class="btn btn-outline-secondary btn-sm w-100 mt-3">
